...
Otro caso relevante para nuestro estudio corresponde al sistema autónomo bidimensional \eqref{eq: sisAutBid}, o sistema planar, definido por funciones polinómicas $f$ y $g$ (denotadas, por ejemplo, como $P(x,y)$ y $Q(x,y)$). A este tipo de sistemas se les denomina \emph{sistemas diferenciales polinómicos} de grado $n$, donde $n$ representa el grado máximo de los polinomios que componen el sistema. Usualmente, se asocian a un campo vectorial $X = P(x,y) \frac{\partial}{\partial x} + Q(x,y) \frac{\partial}{\partial y}$, con $P$ y $Q$ funciones $C^r$ definidas en un abierto $U \subset \R^2$; una notación simplificada es $X = (P,Q)$. No existe distinción entre el campo vectorial $X$ y el sistema polinómico, ya que ambas representan meras formas equivalentes de denotación.
...
\begin{defi}
	Un punto $x_{0} \in \Rn$ es llamada una singularidad, un punto de equilibrio o punto crítico de \eqref{eq: sisAut} si $f(x_{0})=0$. Un punto crítico $x_{0}$ es llamado un punto de equilibrio hiperbólico de \eqref{eq: sisAut} si ninguno de los valores propios de la matriz jacobiana $Df(x_{0})$ tiene parte real cero, en caso contrario, se dice no hiperbólico. El sistema lineal \eqref{eq: sisLinAuto} con la matriz $A = Df(x_{0})$ es llamado la linealización de \eqref{eq: sisAut} en el punto $x_{0}$.
\end{defi}
...
Algo también de relevancia que se puede observar en la \autoref{fig:comparacion}, es que las trayectorias resaltadas dividen el plano de fases en regiones con diferentes comportamientos cualitativos, por ejemplo, la órbita homoclínica de la \autoref{fig: homoclinic} divide el plano de fase en dos regiones: una con un comportamiento hiperbólico y la otra con una dinámica periódica alrededor del punto $(0,1)$. Dichas trayectorias con esta cualidad son denominadas \textit{separatrices}.
...
\begin{equation}
	\begin{aligned}
		\psi : \mathbb{S}^{2} &\longrightarrow \R^{2} \\
...
La bifurcación silla-nodo es el mecanismo básico en el cual puntos de equilibrio son creados y destruidos dependiendo de los parametros usados en el sistema \ref{eq: sisParametro}. En la \autoref{fig: bifSillaNodo} se ejemplifica este tipo de bifurcación.
...
\begin{equation*}
	...
	\end{aligned}
\end{equation*}
Esta invariancia es la expresión matemática de la simetría izquierda-derecha mencionada anteriormente. (En términos más técnicos, se dice que el campo vectorial es equivariante). El retrato fase de este sistema para diferentes valores de $\mu$ se puede visualizar en la \autoref{fig: bifPitchforkSup}.
...
De manera similar que en el supercrítico ahora los puntos críticos solo existen por debajo del valor de bifurcación ($\mu = 0$, en el caso del ejemplo mostrado en la \autoref{fig: bifPitchforkSub}).
...
Supongamos que un sistema bidimensional tiene un punto fijo estable. ¿Cuáles son las posibles maneras en que podría perder estabilidad al variar un parámetro $\mu$.? Los autovalores del Jacobiano son la clave. Si el punto fijo es estable, los autovalores $\lambda _{1}$, $\lambda _{2}$ deben estar en el semiplano izquierdo Re$\lambda <0$. Dado que las $\lambda$'s satisfacen una ecuación cuadrática con coeficientes reales, existen dos posibles escenarios: o bien los autovalores son reales y negativos o bien son conjugados complejos. Para desestabilizar el punto fijo, necesitamos que uno o ambos autovalores crucen al semiplano derecho al variar $\mu$. Al igual que las bifurcaciones en horquilla, las bifurcaciones de Hopf se presentan en variedades supercríticas y subcríticas.
...
\begin{figure}[H]
	...
	\caption{Respuesta funcional tipo IV.}
