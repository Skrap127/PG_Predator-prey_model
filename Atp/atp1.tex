...
\begin{document}
	...
	El presente marco teórico aborda los conceptos fundamentales requeridos para el análisis del modelo presa-depredador tipo Leslie-Gower con respuesta funcional sigmoidea propuesto por González-Olivares et al. (2015) \cite{articulo}.
	...
	\begin{definicion}[Punto de Equilibrio Hiperbólico y No Hiperbólico]
		Un punto de equilibrio es hiperbólico si ninguno de los valores propios de la matriz jacobiana $Df(x_0)$ tiene parte real nula, en caso contrario se le dice no hiperbólico.
	\end{definicion}
	
	\begin{definicion}[Clasificación de Puntos Críticos Hiperbólicos en $\mathbb{R}^n$] \
		\begin{itemize}
	...
	\begin{definicion}[Clasificación de Puntos Críticos No Hiperbólicos en $\mathbb{R}^2$] \
		\begin{enumerate}
			\item \textbf{Basados en Autovalores Imaginarios Puros} (Si los autovalores son $\lambda_{1,2} = \pm i\omega$, con $\omega \neq 0$):
			\begin{itemize}
				\item El punto se denomina \textbf{centro lineal}.
				\item La adición de \textbf{términos no lineales} puede hacer que el punto se convierta en: \textbf{centro}, \textbf{foco} o \textbf{foco-centro}.
			\end{itemize}
			
			\item \textbf{Basados en un Único Autovalor Cero} (Si exactamente un autovalor es $\lambda = 0$):
			\begin{itemize}
				\item El punto se denomina \textbf{semi-hiperbólico}.
				\item Tipos posibles: \textbf{Silla-nodo} o \textbf{Silla Topológica}.
			\end{itemize}
			
			\item \textbf{Basados en Dos Autovalores Cero} (Si ambos autovalores son $\lambda_{1,2} = 0$):
			\begin{itemize}
				\item Estos puntos presentan un \textbf{comportamiento más complejo}.
				\item Tipos posibles: \textbf{focos}, \textbf{centros}, \textbf{nodos}, \textbf{sillas} o \textbf{cúspides}.
			\end{itemize}
		\end{enumerate}
	\end{definicion}
	...
	Analizar cualitativamente el modelo presa-depredador tipo Leslie-Gower con respuesta funcional sigmoidea propuesto por González-Olivares et al. (2015) \cite{articulo}.
	
	\section{Objetivos Específicos}
	
	\begin{itemize}
		\item Hallar los puntos críticos del sistema y analizar su estabilidad local.
		\item Analizar comportamientos cualitativos especiales ya establecidos, como la bifurcación de Hopf, ciclos límites estables y curva separatriz.
	\end{itemize}
	...
	\begin{enumerate}
		...
		\item[E.2] Análisis y síntesis
		\item[E.3] Elaboración del documento final
		\item[E.4] Sustentación
	\end{enumerate}
	...
	\begin{table}[H]
		...
		\begin{tabular}{|p{4cm}|p{3cm}|c|c|c|}
			...
			\multicolumn{4}{|r|}{\textbf{Total 1}} & \textbf{\$886.000} \\ \hline
		\end{tabular}
	...
	\begin{table}[H]
		...
		\begin{tabular}{|p{3cm}|p{2.5cm}|c|c|c|c|}
			...
			Asesoría director & Universidad & 2 & 8 & 48 & \$40.000 \\ \hline
			\multicolumn{5}{|r|}{\textbf{Total 2}} & \textbf{\$1.920.000} \\ \hline
		\end{tabular}
		\caption{Costos de asesoría}
	\end{table}
	
	\vspace{0.5cm}
	\noindent\textbf{Costo aproximado del proyecto:} Total 1 + Total 2 = \$2.806.000
	...
	\begin{table}[H]
		...
		\begin{tabular}{|p{6cm}|c|c|c|c|c|c|}
			...
			Elaboración documento final & & & & & X & X \\ \hline
